...
function renderPrintStepShot(mediaItems) {
  ...
  const renderOne = (m, i) => {
    if (typeof renderProcMediaStack === "function") {
      return `<div class="shot-inner">${renderProcMediaStack(m, { uid: `pr${i}` })}</div>`;
    }
  ...
  if (mediaItems.length === 1) return `<div class="shot">${renderOne(mediaItems[0], 0)}</div>`;
...
function buildAllProceduresPrintDocument() {
  const entries = (typeof allProcedureEntries === "function" ? allProcedureEntries() : []).slice().sort((a, b) => {
  ...
  const sheets = entries.map(({ proc, catalogGammeCode }, i) =>
    buildProcedureFicheSheet(proc, catalogGammeCode, gammeByCode(catalogGammeCode), i + 1, total)
  ).join("\n");
  return `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Procédures atelier OEDIP</title>
...
<div class="toolbar"><button class="btn" onclick="window.print()">Imprimer / PDF</button><p>${entries.length} fiche(s) — toutes gammes</p></div>
...
function renderProcedureFicheEditor(proc, gam) {
  ...
  return `<p class="hint">Format <span class="mono">template_fiche_procedure.html</span> — une page A4 par procédure.</p>
